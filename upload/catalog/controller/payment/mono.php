...
class ControllerPaymentMono extends Controller {
    ...
    public function __construct($registry) {
        ...
        try {
            ...
            clientHandleException($e, $this->model_payment_mono, true);
        }
    }
    ...
    public function index() {
        ...
        try {
            if (!array_key_exists(strtoupper($order_info['currency_code']), $this->CURRENCY_CODE)) {
                return;
            }
...
//        it's not pretty, але маємо те шо маємо
            ...
            if ($default_ccy != 'UAH' && $order_info['currency_code'] == 'UAH') {
                ...
                if (isset($rates['created']) && $rates['created'] >= self::RATE_CACHE_TIMEOUT_SEC) {
                    ...
                    foreach ($rates['rates'] as $rate) {
                        if ($rate['currencyCodeA'] == $this->CURRENCY_CODE[$default_ccy] && $rate['currencyCodeB'] == 980) {
    ...
    public function response() {
        ...
        switch ($order_info['order_status_id']) {
            ...
            case '0':
            {
                $invoice_db = $this->model_payment_mono->InvoiceGetLastByOrderId($order_id);
                if ($invoice_db == null) {
...
//                        something is wrong
                ...
                switch ($invoice_db['status']) {
                    case 'success':
                        $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mono_order_success_status_id'), $this->language->get('text_status_success'), true);
                        $this->cart->clear();
                        $this->response->redirect($this->url->link('checkout/success', '', true));
                        break;
                    case 'hold':
...
//                        $this->model_checkout_order->addOrderHistory($orderID, $this->config->get('mono_order_process_status_id'), $this->language->get('text_status_process'), true);
    ...
    public function callback() {
        ...
        $final_amount = (key_exists('finalAmount', $status_response)) ? $status_response['finalAmount'] : 0;
        $invoice_db = $this->model_payment_mono->InvoiceGetById($status_response['invoiceId']);
        ...
        if ($need_insert_invoice) {
            $mono_order = $this->model_payment_mono->getOrderInfo($status_response['invoiceId']);
            ...
            $invoice_db = [
                ...
                'order_ccy' => $this->CURRENCY_CODE[$order['currency_code']],
                'order_amount' => (int)($order['total'] * 100 + 0.5),
                'payment_amount' => $invoice_status['status'] == 'created' ? 0 : $invoice_status['amount'],
        ...
        if (!$need_insert_invoice) {
            $this->model_payment_mono->InvoiceUpdateStatus($status_response['invoiceId'],
        ...
        switch ($status_response['status']) {
            ...
            case 'success':
                ...
                break;
            case 'hold':
                if ($order['order_status_id'] != $this->config->get('mono_order_hold_status_id')) {
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mono_order_hold_status_id'), $this->language->get('text_status_hold'), true);
                }
                break;
            case 'processing':
                if ($prev_invoice_status == 'created') {
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mono_order_process_status_id'),
            ...
            case 'reversed':
                ...
                if ($order['order_status_id'] == $this->config->get('mono_order_success_status_id')) {
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mono_order_success_status_id'),
                        $this->language->get('text_status_refund'));
                } else if ($order['order_status_id'] == $this->config->get('mono_order_hold_status_id')) {
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mono_order_cancelled_status_id'),
    ...
    public function logs() {
        ...
        $logs = $this->model_payment_mono->SelectLogs($from, $limit, $offset);
    ...
    function getCheckoutUrl($order_info, $ccy) {
        $payment_type = 'debit';
        $hold_status = $this->config->get('mono_use_holds');
        if ($hold_status == 1) {
            $payment_type = 'hold';
        }
        $create_invoice_response = $this->createInvoice($order_info, $payment_type, $ccy);
        $this->model_payment_mono->InvoiceInsert($create_invoice_response['invoiceId'], $order_info['order_id'], $payment_type, (int)($order_info['total'] * 100 + 0.5), 0, 0, 'created', $this->CURRENCY_CODE[$ccy], 0);
        return $create_invoice_response['pageUrl'];
    }

    function createInvoice($order_info, $payment_type, $ccy) {
        ...
        curl_setopt_array($curl, [
            ...
            CURLOPT_POSTFIELDS => json_encode([
                ...
                'webHookUrl' => str_replace('&amp;', '&', $this->removeMiddlePath($this->url->link('payment/mono/callback'))),
            ]),
            CURLOPT_HTTPHEADER => [
                'Content-Type: application/json',
                'X-Token: ' . $this->config->get('mono_merchant'),
    ...
    function removeMiddlePath($url) {
        ...
        // Add the query and fragment if they exist
    ...
    function insertInvoiceFromOldTable($order_info) {
        try {
            $mono_order = $this->model_payment_mono->getOrder($order_info['order_id']);
        } catch (\Exception $e) {
        ...
        $invoice_db = [
            ...
            'payment_amount' => $status_response['status'] == 'created' ? 0 : $status_response['amount'],
            'payment_amount_refunded' => $amount_refunded,
            'payment_amount_final' => $final_amount,
    ...
    public function getEncodedProducts($order_id, $order_total) {
        $order_products = $this->model_payment_mono->getOrderProducts($order_id);
        $product_ids = [];
        foreach ($order_products as $orderProduct) {
            $product_ids[] = $orderProduct['product_id'];
        }
        $products = $this->model_payment_mono->getProducts($product_ids);
        $products_map = [];
...
//            splitting multiple qty items so that discount coupons will be correctly applied to prices
        ...
        if ($total_from_basket != $order_total) {
            $totals = $this->model_payment_mono->getTotals($order_id);
